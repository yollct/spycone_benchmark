...
import pandas as pd
from scipy.stats import dirichlet, multinomial, norm, invgamma, poisson, beta, multivariate_normal, halfcauchy, halfnorm, gamma, nbinom
import random
...
class eqHMM():
    def __init__(self, ngenes, nclusters, nobs, reps=3, n_states=5, p_init=None, p_trans=None, isoformsdata=None, output=None, noise_level=0.5):
        self.p_init = p_init
...
#                                                      '{}i{}'.format(gene,niso+1), 'no'] + list(noise_emi)
    ...
    def swap_random(self, arr):
        idx = np.where(arr>0.3)[0]
        if len(idx)>2:
            arr1 = arr.copy()
            i1, i2 = random.sample(set(idx), 2)
    ...
    def simulation(self):
        ...
        for j in range(self.ngenes):
            ...
            if niso >= 3:
                ...
                iso_switch_prob = np.arange(1, (int(n_switched))+1)*10
...
#                 iso_dist1 = dirichlet.rvs(np.append([1]*(niso-n_switched), self.swap_random(iso_switch_prob)))[0]
                ...
                iso_abun1, i1, i2 = self.swap_random(iso_abun0)
                ...
                #set the iso dist for each state    
                iso_dist0 = np.arange(1, (niso+1))*10
                iso_dist1, i1, i2 = self.swap_random(iso_dist0)
                
                #abun
                iso_abun0 = dirichlet.rvs(iso_dist0)[0] 
                iso_abun1 = dirichlet.rvs(iso_dist1)[0] 
                iso_emi0 = iso_abun0 * float(gene_mean)
                iso_emi1 = iso_abun1 * float(gene_mean)
                vars0 = [self.get_var((iso_emi0[x]*self.noise_level)/(self.noise_level+iso_emi0[x])) for x in range(len(iso_abun0))]
                vars1 = [self.get_var((iso_emi1[x]*self.noise_level)/(self.noise_level+iso_emi1[x])) for x in range(len(iso_abun0))]
